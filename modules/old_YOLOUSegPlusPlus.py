...
class CBAMLiteFusion(nn.Module):
    def __init__(self, in_channels):
        ...
        self.spatial_proj = nn.Conv2d(in_channels+1, 1, 5, padding=2, bias=False)  
        self.conv = nn.Conv2d(
                in_channels = 1,
                out_channels = 1, 
                kernel_size = 1)
    ...
    def forward(self, skip, heatmap):
        channel_gate = self.channel_proj(skip)
        spatial_gate = self.spatial_proj(torch.cat([skip, self.conv(heatmap)], dim=1))
        gate = channel_gate * spatial_gate
        # return heatmap + gate * skip  
        return gate * skip + skip
...
class YOLOUSegPlusPlus(Module): 
    def __init__(self,
        ...
        ### (256 Channel) Full-Size Set-up UNet-like (without concat_proj)
        self.bottleneck = Sequential(
            SpatialTransformer(256), 
            DoubleConv(256, 256), # FUTURE: Replace it with DoubleLightConv and See Performance Loss
        )
        self.decoder = nn.ModuleList([
        Sequential(
            DoubleConv(256, 256), # FUTURE: Decrease and See Performance Loss + (1) Heatmap
            DoubleLightConv(256, 256)
        ),
        DoubleLightConv(256, 128),
        Sequential(
            DoubleConv(256, 160), # -> (128) in + (128) Skip 
            DoubleLightConv(160, 128)
         ),
         DoubleLightConv(128, 64),
         Sequential(
            DoubleConv(192, 128), # -> (64) in + (128) Skip + (1) Heatmap
            DoubleLightConv(128, 64)
        ),
        DoubleLightConv(64, 32), 
        Sequential(
            DoubleConv(96, 64),   # -> (32) in + (64) Skip 
            DoubleLightConv(64, 32)
        ),
        DoubleLightConv(32+1, 16),
        ])

        self.output = nn.Conv2d(in_channels=16, out_channels=1, kernel_size=1) # FUTURE: Replace it with DoubleLightConv
        self.eca = nn.ModuleList([ECA() for i in range(len(target_modules_indices) + 1)])
        self.gaussian = GaussianBlur(kernel_size=5)
...
#             DoubleConv(192, 192),
#         ),
#         DoubleLightConv(192, 96),
#         Sequential(
#             GhostBlock(96, 96),
#             DoubleConv(96, 96),
#          ),
#          DoubleLightConv(96, 48),
#          Sequential(
#             GhostBlock(48, 48),
#             DoubleConv(48, 48),
#         ),
#         DoubleLightConv(48, 24),
#         Sequential(
#             GhostBlock(24, 24),  
#             DoubleConv(24, 24),
#         ),
#         DoubleLightConv(24, 12),
#         ])
...
# 
#         self.output = nn.Conv2d(in_channels=12, out_channels=1, kernel_size=1)

        # self.output = nn.Conv2d(in_channels=16, out_channels=1, kernel_size=1)
        

        ### 256 Channel Full-Size Set-up
        # self.bottleneck = Sequential(
        #     SpatialTransformer(256), 
        #     DoubleConv(256, 256),
        # )
        ...
        for idx, module in enumerate(self.decoder): 
            if idx in self._indices.get("upsample"): 
                ...
                if idx == 7: 
                    ...
                    heatmap = self.alpha[0] * heatmap
                    x = torch.concat([x, heatmap], dim=1)        
...
# heatmap = self.alpha[0] * heatmap + (1 - self.alpha[0])

                # Heatmap Preprocessing
                
                # if std > 0: heatmap = (heatmap - heatmap.mean()) / heatmap.std()
                # else: heatmap = heatmap - heatmap.mean() # Avoid NaN if std = 0
                ...
                # if idx == 4:
                    ...
                    # heatmap = self.sigmoid(heatmaps.pop())
                    # heatmap = (heatmap - heatmap.min()) / (heatmap.max() - heatmap.min() + 1e-6)
                    # heatmap = (heatmap > 0.5).float()
                    # heatmap = self.gaussian(heatmap)

                    
                    # print(skip.size(), heatmap.size())
                    # skip = skip + (heatmap * skip)
                    # skip = skip * (1 + self.alpha[al] * heatmap) + self.beta[be] * heatmap ; al+=1 ; be+=1
                    
                    # skip = torch.concat([skip, heatmap], dim=1)
...
#                     
#                     x = torch.concat([x, skip, heatmap], dim=1)
