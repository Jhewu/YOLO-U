...
if __name__ == "__main__": 
    ...
    x = cv2.imread("BraTS-SSA-00002-00030-t1c_image.png", cv2.IMREAD_UNCHANGED)
    ...
    print(a[:, -1:])
    print()
    print(torch.sigmoid(a[:, -1:]))
    print(type(a[:, -1:]))

    import matplotlib.pyplot as plt

    plt.imshow(torch.sigmoid(a[:, -1:]).squeeze(0).squeeze(0).cpu().numpy())
    plt.show()
